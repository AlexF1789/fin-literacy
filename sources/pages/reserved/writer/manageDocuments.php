...
<html lang="en-US">
    <head>
        <!-- TITLE AND METAS -->
        <title>Financial Literacy - manage documents</title>
        <meta charset="UTF-8"/>
        ...
        <section>
            <article>
                ...
                <?php
                    ...
                    $query = query("SELECT object, link, id FROM documents WHERE author='$mail' ORDER BY id DESC");
                    if($query->num_rows) {
                        echo "<table><tr><td></td><td></td><td></td></tr>";
                        while($a=mysqli_fetch_row($query)) {
                            $ext = end(explode(".", $a[1]));
                            echo "<tr><td style='width: 2vW'><img src='/sources/images/fileIcons/".$ext."_icon.png' type='image/png' class='fileIcon'></td><td><a href='/sources/docs/published/$a[1]' class='link' target='_blank'>$a[0]</a></td><td><a class='redLink' href='/sources/pages/auto/removeDocument.php?id=$a[2]'>delete</a></td></tr>";
                        }
                        echo "</table>";
                    } else
                        echo "<p style='color: red; font-weight: bold;'>You haven't published documents yet!</a>";
    ...
    <script type="text/javascript">
        message = new Array("","The document was correctly deleted.","There was an error deleting the document; try again later.");
        type = new Array("","success","error");
    </script>
</html>