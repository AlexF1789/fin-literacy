<?php
    ...
    if(!isset($_SESSION['mail']) || $_SESSION['platform']!='fin-literacy' || $_SESSION['role']!=1)
        header("location: /");
...
<html lang="en-US">
    <head>
        <!-- TITLE AND METAS -->
        <title>Financial Literacy - your articles</title>
        <meta charset="UTF-8"/>
    ...
    <body>
        ...
        <section>
            <article>
                <h1>Your Articles</h1>
                <p>Here you can find the index of the articles you've pubblished ordered by publication date.</p><br>
                
                <?php

                    $query = query("SELECT id, title FROM articles WHERE author='$mail' ORDER BY id DESC");
                    if($query->num_rows) {
                        echo "<ul>";
                        while($a=mysqli_fetch_row($query)) {
                            echo "<li>&ensp;<a href='/sources/pages/articles/article.php?id=$a[0]' class='link'>$a[1]</a><br><a class='redLink' onclick='delArticle(\"$a[0]\")'>delete</a><br><a class='link' onclick='addImage(\"$a[0]\")'>add images</a><br><a href='/sources/pages/reserved/writer/editArticle.php?idArt=$a[0]' class='link'>edit article</a><br><br></li>";
                        }
                        echo "</ul>";
                    } else
                        echo "<p style='color: red; font-weight: bold;'>You haven't pubblished articles yet!</a>";
    ...
    <script type="text/javascript">
        message = new Array("","The article has correctly been deleted.","There was an error deleting the article.","The article has correctly been pubblished.","There was an error pubblishing the article.","You've correctly uploaded the image.","There was an error uploading the image.","The article has correctly been edited","There was an error editing the article.");
        type = new Array("","success","error","success","error","success","error","success","error")
    </script>
</html>