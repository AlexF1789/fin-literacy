<?php
    ...
    if(!isset($_SESSION['mail']) || $_SESSION['platform']!='fin-literacy' || $_SESSION['role']!=1 || !isset($_GET['idArt']) || !query("SELECT * FROM articles WHERE id='$idArt' AND author='$mail'")->num_rows)
        header("location: /");

    $query = query("SELECT title, text FROM articles WHERE id='$idArt' AND author='$mail'");
    if($query->num_rows) {
...
<html lang="en-US">
    <head>
        <!-- TITLE AND METAS -->
        <title>Financial Literacy - edit article</title>
        <meta charset="UTF-8"/>
    ...
    <script type="text/javascript" src="/sources/script/nicEdit.js"></script>
    ...
    <body>
        ...
        <section>
            <article>
                <h1>Edit Article</h1>
                <p>Insert the title and the text of the article (which can be formatted by pressing the icons in the textbox) to edit the article; if you desire to add images you can add them in a second time by pressing the <i>add images</i> link in the <i>Your articles</i> section.<br><br><b>IMPORTANT ADVICE</b>&emsp;frequently have a backup of the article so as to avoid the risk of losing it.</p><br>
                
                <form method="POST" action="/sources/pages/auto/editArticle.php">

                    <input type="hidden" readonly="readonly" name="idArt" required value="<?php echo $idArt ?>">
                    <input type="text" class="textField" name="title" placeholder="Article title" style="width: 60%" required value="<?php echo $title ?>">
                    <br><br><p>Article content</p>
                    <textarea style="width: 100%; height: 25vW;" name="text"><?php echo $text ?></textarea>

                    <br><br>
                    <center>
                        <input type="submit" value="Edit article" class="button">
                    </center>
                </form>
