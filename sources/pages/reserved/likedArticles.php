...
<html lang="en-US">
    <head>
        <!-- TITLE AND METAS -->
        <title>Financial Literacy - liked articles</title>
        <meta charset="UTF-8"/>
    ...
    <body>
        ...
        <section>
            <article>
                ...
                <p>Here you can find the index of the articles you liked ordered by the date you liked them.</p><br>
                
                <?php

                    $query = query("SELECT articles.id, articles.title FROM articles, articlesLike WHERE articlesLike.idArt=articles.id AND articlesLike.mail='$mail' ORDER BY articlesLike.id DESC");
                    if($query->num_rows) {
                        echo "<ul>";
