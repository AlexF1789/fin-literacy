<?php
    ...
    require "<path to the php folder>/php/data.php"; // path to the php folder in your web sever

    if(!isset($_GET['mail']) || !isset($_GET['code'])) {
        header("location: /souces/pages/login.php");
    } else {
        $mail = $_GET['mail'];
        $code = $_GET['code'];

        $query = query("SELECT name, mail FROM users WHERE mail='$mail' AND verCode='$code'");
        if($query->num_rows) {
...
<html lang="en-US">
    <head>
        <!-- TITLE AND METAS -->
        <title>Financial Literacy - Recover password</title>
        <meta charset="UTF-8"/>
    ...
    <body>
        ...
        <section>
            <article>
                ...
                <p>Hi <?php echo $name." ($mail)" ?>, now you can edit your password by writing the new one two times (one time for each text field).</p>

                <form method="POST" action="/sources/pages/auto/sendRecoverPwd.php" class="login">
                    <input name="password" type="password" placeholder="Password" required class="textField"><br>
                    <input name="passwordCheck" type="password" placeholder="Confirm Password" required class="textField"><br>
                    <input name="mail" type="hidden" value="<?php echo $mail ?>" readonly="readonly" required>
                    <input type="submit" class="button" value="Change Password" title="Recover your password by changing it">
                </form>
                <br>
            </article>
        </section>
